...
export function useFlaringAssets(p: { assetId?: string; period?: string } = {}) {
  return useQuery({
    ...
    queryFn: ({ signal }) => flaringApi.assets(p, signal),
  });
}
...
export function useMethaneIntensity(p: { assetId?: string; period?: string } = {}) {
  return useQuery({
    ...
    queryFn: ({ signal }) => flaringApi.methaneIntensity(p, signal),
  });
}
...
export function useZeroRoutineTracker(p: { assetId?: string } = {}) {
  return useQuery({
    ...
    queryFn: ({ signal }) => flaringApi.zeroRoutineTracker(p, signal),
  });
}
...
export function useGasOpportunity(p: { assetId?: string; period?: string } = {}) {
  return useQuery({
    ...
    queryFn: ({ signal }) => flaringApi.opportunity(p, signal),
  });
}
