...
/**
 * Route protection (Next 16 "proxy" convention — formerly middleware). Thin adapter: pull the
 * session cookie + path off the request, defer to the pure `routeDecision` (unit-tested in
 * lib/auth/__tests__), and apply the result. All real authorization still happens
 * backend-side on data calls — this only gates navigation and bounces signed-in users off
 * the auth pages.
 */
